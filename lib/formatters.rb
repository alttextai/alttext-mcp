...
module Formatters
  ...
  def format_image(image)
    ...
    lines << "Created: #{Time.at(image['created_at']).utc.iso8601}" if image['created_at'].is_a?(Numeric)

    errors = image['errors'] || {}
    lines << "Errors: #{errors.values.flatten.join(', ')}" if errors.any?
  ...
  def format_scrape_result(result, url)
    ...
    lines << "\nErrors: #{errors.values.flatten.join(', ')}" if errors.any?
